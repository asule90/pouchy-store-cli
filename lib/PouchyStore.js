...
import generateReplicationId from './generateReplicationId.js';
...
export default class PouchyStore {
  ...
  async initialize() {
    ...
    try {
      this.dbLocal = new PouchDB(DB_DIR+this.name, {
        auto_compaction: true,
        revs_limit: 2,
        ...this.optionsLocal,
      });
      this.dbMeta = new PouchDB(`${DB_DIR}${PREFIX_META_DB}${this.name}`, {
        auto_compaction: true,
        revs_limit: 2,
      });
      if (this.isUseRemote) {
    ...
    if (this.isUseRemote) {
      ...
      try {
        await checkInternet(this.urlRemote).then(async () => {
          await this.dbLocal.replicate.from(this.dbRemote, {
            ...
            console.e('replication is complete');
            // console.e(info);
          });
          // this.dbLocal.info().then(function (info) {
          //   console.e('DB doc total: '+info.doc_count);
          // })
          await this.initUnuploadeds();
        }).catch(err => {
          //no connection
          console.e(err.message);
        });
      } catch (err) {
        console.e(err);
      }
  ...
  async initUnuploadeds() {
    ...
    try {
      ...
      // console.e('replicationId: '+replicationId)
      const replicationDoc = await this.dbLocal.get(replicationId)
        .catch(function (err) {
          // console.e('error retrieving doc');
          // console.e(err);
        });
      // console.e('replicationDoc: '+replicationDoc)
      const unuploadeds = await this.dbLocal.changes({
        since: (replicationDoc && replicationDoc.last_seq ? replicationDoc.last_seq : 'now'),
        include_docs: true,
      });
  ...
  upload() {
    const _this = this;
    if (!_this.isUseRemote) return;

    return checkInternet(_this.urlRemote).then(async () => {
      await _this.dbLocal.replicate.to(_this.dbRemote);
      const ids = Object.keys(_this.dataMeta.unuploadeds);
      for (let id of ids) {
        delete _this.dataMeta.unuploadeds[id];
      }
      _this.dataMeta.tsUpload = new Date().toJSON();
      _this.persistMeta();
